...
router.get('/login', async (ctx, next)=> {
    ...
    try {
        ...
        if(!code || typeof code !== 'string' || !code.length) {
            ...
            ctx.cookies.set('authorization', token, { httpOnly: true, domain: process.env.CLIENT_ID });
            setResponse(ctx, { data :true, status: StatusCodes.OK });
        }
    } catch (error) {
        console.error(error);
...
router.get('/logout', async (ctx, next) => {
    ctx.cookies.set('authorization', '', { httpOnly: true, domain: process.env.CLIENT_ID });
    setResponse(ctx, { data: null, status : StatusCodes.OK });
    return await next();
});
